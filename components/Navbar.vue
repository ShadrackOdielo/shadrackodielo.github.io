<template>
  <header :class="{'bg-white dark:bg-gray-800': !isHomeSection, 'bg-transparent': isHomeSection}" class="fixed w-full z-50 transition-colors duration-300">
    <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
      ...
      <div class="hidden md:flex space-x-4">
        <a v-for="link in links" :key="link.id" :href="link.href" class="text-black dark:text-white hover:text-blue-500" :class="{'underline': activeSection === link.id}">{{ link.name }}</a>
